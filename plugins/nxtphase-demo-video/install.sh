...
# `<scope>/skills/nxtphase-demo-video/`.
#
# The end card font is not shipped here; see README.md.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_SRC="$SCRIPT_DIR/skills/nxtphase-demo-video"
SKILL_NAME="nxtphase-demo-video"
...
if [[ "$ACTION" == "uninstall" ]]; then
    rm -rf "$SKILL_DST"
    echo "nxtphase-demo-video uninstalled from $SKILL_DST"
    exit 0
fi
...
echo "nxtphase-demo-video installed into $SKILL_DST"
echo "  SKILL.md, reference/, assets/"
echo
echo "Note: PPEditorialNew-Italic.otf is not included. Fetch it internally and"
echo "put it in public/fonts/ of your video project, see README.md."
echo
